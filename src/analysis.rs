...
pub fn save_focus_commits(opts: &env::Options, filename: &str, res: &HashSet<(String, String, String, String, String)>) -> bool{
    let prefix: String;
    ...
    let Ok(mut file_w) = File::create_new(format!("{}/{}", prefix, filename))
        else {
...
pub fn focus_missing_commits_all_files_with_save(
    opts: &env::Options
) -> bool
{
    let prefix: String;
    ...
    <G as SwhGraphWithProperties>::LabelNames: swh_graph::properties::LabelNames,
{
    ...
    missing_commits
        ...
        .for_each(|commit|{
            ...
            graph.successors(graph.properties().node_id(commit.3.as_str()).unwrap())
                .into_iter()
                .for_each(|succ|{
                    let swhid = graph.properties().swhid(succ).to_string();
                    if env::RE_REV.is_match(&swhid){
                        no_common = false;
                        res
                            .entry((commit.0.to_owned(), commit.1.to_owned(), commit.2.to_owned(), commit.3.to_owned(), commit.4.to_owned()))
...
pub fn first_revision_different<G: SwhLabeledForwardGraph + SwhGraphWithProperties>(
    map_common_commits: HashMap<(String, String, String, String, String), HashSet<(String, String, String)>>,
    graph: &G
) -> HashMap<(String, String, String, String, String), HashMap<(String, String, String), HashSet<String>>>
// return Set(diff revision)
where
    <G as SwhGraphWithProperties>::Maps: swh_graph::properties::Maps,
    <G as SwhGraphWithProperties>::LabelNames: swh_graph::properties::LabelNames,
{
    let mut ret_value : HashMap<(String, String, String, String, String), HashMap<(String, String, String), HashSet<String>>> = HashMap::new();
    ...
    for (missing, common_commits) in map_common_commits.iter(){
        ...
        let common_commits_rev: HashSet<String> = common_commits.into_iter().map(|c| c.1.to_owned()).collect();
    ...
    <G as SwhGraphWithProperties>::LabelNames: swh_graph::properties::LabelNames,
{
    ...
    for ori in env::ORIGINS_LIST_2021.iter(){
        ...
        .into_iter()
        .for_each(|commit| {
            println!("{};{};{};{};{}", commit.0, commit.1, commit.2, commit.3, commit.4);
            res.insert(commit);
        }); 
    }
    return res;
...
pub fn load_file_results(opts: &env::Options, filename: &str) -> Option<HashSet<(String, String, String, String, String)>>{
    let prefix: &str;
    match opts.dataset.as_str(){
